...
import backtype.storm.scheduler.SingleTopologyCluster;
...
public class ResourceAwareScheduler implements IScheduler {
    ...
    public void prepare(Map conf) {
        this.conf = conf;
        schedulingPriorityStrategy = Utils.newInstance(
                (String) conf.get(Config.RESOURCE_AWARE_SCHEDULER_PRIORITY_STRATEGY));
        ...
        maxSchedulingAttempts = Utils.getInt(
            conf.get(Config.RESOURCE_AWARE_SCHEDULER_MAX_TOPOLOGY_SCHEDULING_ATTEMPTS), 5);
    }
    ...
    private void scheduleTopology(TopologyDetails td, Cluster cluster, final User topologySubmitter,
        ...
        try {
            rasStrategy = Utils.newSchedulerStrategyInstance((String) td.getConf().get(Config.TOPOLOGY_SCHEDULER_STRATEGY), conf);
    ...
    private Map<String, Map<String, Double>> getUserResourcePools() {
        ...
        raw = conf.get(Config.RESOURCE_AWARE_SCHEDULER_USER_POOLS);
