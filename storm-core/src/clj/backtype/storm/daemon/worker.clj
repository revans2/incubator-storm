...
(defserverfn mk-worker [conf shared-mq-context storm-id assignment-id port worker-id]
  ...
  (let [storm-conf (read-supervisor-storm-conf conf storm-id)
        ...
        cluster-state (cluster/mk-distributed-cluster-state conf :auth-conf storm-conf :acls acls :context (ClusterStateContext. ClusterStateContext$DaemonType/WORKER))
