         ...
         (let [storm-ids (get-worker-hb-children WORKERBEATS-SUBTREE pacemaker-client)]
           (doseq [id storm-ids]
             (let[hb-paths (get-worker-hb-children
                             (str WORKERBEATS-SUBTREE util/file-path-separator id) pacemaker-client)
                  _ (log-debug "hb-paths" hb-paths)]
               (doseq [path hb-paths]
                 (let [wk-hb (clojurify-details (get-worker-hb (str WORKERBEATS-SUBTREE
                                                                 util/file-path-separator id
                                                                 util/file-path-separator path)
                                                               pacemaker-client))]
                   (when (and wk-hb (> 600 (- (util/current-time-secs) (:time-secs wk-hb))))
