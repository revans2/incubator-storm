...
class Client(Iface):
  ...
  def getSchedulerConf(self):
    self.send_getSchedulerConf()
    return self.recv_getSchedulerConf()

  def send_getSchedulerConf(self):
    self._oprot.writeMessageBegin('getSchedulerConf', TMessageType.CALL, self._seqid)
    args = getSchedulerConf_args()
    args.write(self._oprot)
    self._oprot.writeMessageEnd()
    self._oprot.trans.flush()

  def recv_getSchedulerConf(self):
    iprot = self._iprot
    (fname, mtype, rseqid) = iprot.readMessageBegin()
    if mtype == TMessageType.EXCEPTION:
      x = TApplicationException()
      ...
      iprot.readMessageEnd()
      raise x
    result = getSchedulerConf_result()
    result.read(iprot)
    iprot.readMessageEnd()
    if result.success is not None:
      return result.success
    if result.aze is not None:
      raise result.aze
    raise TApplicationException(TApplicationException.MISSING_RESULT, "getSchedulerConf failed: unknown result")
...
class Processor(Iface, TProcessor):
  def __init__(self, handler):
    ...
    self._processMap["getSchedulerConf"] = Processor.process_getSchedulerConf
  ...
  def process_getSchedulerConf(self, seqid, iprot, oprot):
    args = getSchedulerConf_args()
    args.read(iprot)
    iprot.readMessageEnd()
    result = getSchedulerConf_result()
    try:
      result.success = self._handler.getSchedulerConf()
      msg_type = TMessageType.REPLY
    except (TTransport.TTransportException, KeyboardInterrupt, SystemExit):
      raise
    except AuthorizationException as aze:
      ...
      logging.exception(ex)
      result = TApplicationException(TApplicationException.INTERNAL_ERROR, 'Internal error')
    oprot.writeMessageBegin("getSchedulerConf", msg_type, seqid)
    result.write(oprot)
    oprot.writeMessageEnd()
    oprot.trans.flush()
...
class getSchedulerConf_args:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('getSchedulerConf_args')
    oprot.writeFieldStop()
    oprot.writeStructEnd()
...
class getSchedulerConf_result:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('getSchedulerConf_result')
    if self.success is not None:
      oprot.writeFieldBegin('success', TType.STRING, 0)
      oprot.writeString(self.success.encode('utf-8'))
      oprot.writeFieldEnd()
