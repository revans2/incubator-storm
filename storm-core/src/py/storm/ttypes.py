...
class AccessControl:
  ...
  def __hash__(self):
    return 0 + hash(self.type) + hash(self.name) + hash(self.access)
...
class ReadableBlobMeta:
  ...
  def __hash__(self):
    return 0 + hash(self.settable) + hash(self.version)
...
class ListBlobsResult:
  ...
  def __hash__(self):
    return 0 + hash(self.keys) + hash(self.session)
...
class BeginDownloadResult:
  ...
  def __hash__(self):
    return 0 + hash(self.version) + hash(self.session)
...
class HBExecutionException(Exception):
  """
  Attributes:
   - msg
  """
...
class Pulse:
  ...
  def __hash__(self):
    return 0 + hash(self.id) + hash(self.details)
