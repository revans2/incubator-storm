...
public class NormalDistStats implements Serializable {
    ...
    public static NormalDistStats fromConf(Map<String, Object> conf, Double def) {
        ...
        double mean = Utils.getDouble(conf.get("mean"), def);
        double stddev = Utils.getDouble(conf.get("stddev"), mean / 4);
        double min = Utils.getDouble(conf.get("min"), 0.0);
        double max = Utils.getDouble(conf.get("max"), Double.MAX_VALUE);
