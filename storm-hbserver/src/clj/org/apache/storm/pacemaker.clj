...
(defn get-all-nodes-for-path [^String path heartbeats]
...
(defn mk-service [conf]
  (let [min (conf HBSERVER-BASE-THREADS)
        max (conf HBSERVER-MAX-THREADS)
        mins-wait (conf HBSERVER-THREAD-TIMEOUT)
